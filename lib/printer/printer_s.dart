...
class PrinterService {
  ...
  Future<void> testPrint({
    ...
    if (type == PrinterType.usb) {
      ...
      final printObject = _usbService.samplePrintObject(
      ...
      await _usbService.printData(printer: printer, printObject: printObject);
      return;
    }
    ...
    if (type == PrinterType.internal) {
      if (orderId > 0) {
        try {
          ...
          final data = res.data;
          final printObjects = _extractBackendPrintObjects(data);
  ...
  Future<void> _printWithSunmi(List<Map<String, dynamic>> printObject) async {
    ...
    for (final item in printObject) {
      if (item['type'] == 'text') {
        final options =
            item['options'] is Map ? Map<String, dynamic>.from(item['options']) : {};
...
List<Map<String, dynamic>> _buildReceiptIsolate(
  ...
  final dt = millis != null
      ...
      : DateTime.now();
  final dateTime =
      "${dt.year}-${_twoIsolate(dt.month)}-${_twoIsolate(dt.day)} "
  ...
  final String restaurantName =
      (args["restaurantName"] ?? "SELFX").toString();
